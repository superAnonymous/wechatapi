...
public class NewsOutMessage extends OutEnterpriseMessage {
	...
	public String toXmlString() {
		...
		root.addElement(OetpmUtils.PKG_NEWS_ARTICLECOUNT).addText(Integer.toString(articles.size()));
		Element articlesElement = root.addElement(OetpmUtils.PKG_NEWS_ARTICLES);
		for(Article article : articles) {
			Element item = articlesElement.addElement(OetpmUtils.PKG_NEWS_ITEM);
			if (! StringUtils.isEmpty(article.getTitle())) {
				item.addElement(OetpmUtils.PKG_NEWS_TITLE).addCDATA(article.getTitle());
			}
			if (! StringUtils.isEmpty(article.getDescription())) {
				item.addElement(OetpmUtils.PKG_NEWS_DESCRIPTION).addCDATA(article.getDescription());
			}
			if (! StringUtils.isEmpty(article.getPicUrl())) {
				item.addElement(OetpmUtils.PKG_NEWS_PICURL).addCDATA(article.getPicUrl());
			}
			if (! StringUtils.isEmpty(article.getUrl())) {
				item.addElement(OetpmUtils.PKG_NEWS_URL).addCDATA(article.getUrl());
			}
		}
		return root.asXML();
	}
